...
- Wait for the container to build and when it is ready it will pop up a terminal where you can run `mvn`, `java`, `build.sh` etc.

Changes you make in the source code will be reflected immediately in the host, and the container has been configured to share the Maven cache with the host, so builds should be fast.
